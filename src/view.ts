 ...
 *
 * The read tools in this server hand back Crowntown Compost's payload close to
 ...
 * honestly say which of Crowntown Compost's fields matter and which are noise.
 *
 ...
 */
export const CTC_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Crowntown Compost\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Crowntown Compost fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(CTC_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, CTC_VIEWS);
