...
// with CROWNTOWN_USERNAME + CROWNTOWN_PASSWORD set would read and write
// ~/.crowntowncompost-mcp/session.json — non-hermetic, order-dependent, and able to
...
const CACHE_DIR = mkdtempSync(join(tmpdir(), 'ctc-test-cache-'));

beforeEach(() => {
  process.env.CROWNTOWN_SESSION_CACHE = 'false';
  process.env.CROWNTOWN_SESSION_FILE = join(CACHE_DIR, 'session.json');
});

afterAll(() => {
  rmSync(CACHE_DIR, { recursive: true, force: true });

  const leaked = join(homedir(), '.crowntowncompost-mcp');
  if (existsSync(leaked)) {
    ...
    throw new Error(
      `A test wrote to ${leaked}. The suite must never touch the real home ` +
        'directory — inject CROWNTOWN_SESSION_CACHE=false (or a temp ' +
        'CROWNTOWN_SESSION_FILE) into the env that test hands the client.',
    );
  }
});
